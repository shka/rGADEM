...
To load the rGADEM package, you should use this command:
<<loading rGADEM package>>=
library(rGADEM)
@
In the case of your data is provided from Homo Sapiens: 
...
We are ready to start seeded analysis \newline
...
\subsection{Example of background model}
